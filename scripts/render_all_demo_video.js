// ==============================================================================
// ARISE PRODUCTION STUDIO - COMPLETE VIDEO GENERATOR
// ==============================================================================
...
function renderAllSlides() {
  console.log('🎨 [1/3] Generating 1080p HTML slides and rendering sequentially...');
  ...
  for (const slide of SLIDES) {
    ...
    const userDir = `/tmp/arise_demo_p_${slide.id}`;
    
    fs.writeFileSync(htmlFile, generateSlideHtml(slide), 'utf8');

    if (fs.existsSync(pngFile) && fs.statSync(pngFile).size > 100000) {
      console.log(`⏩ Slide ${slide.id}/14 already rendered (${pngFile}). Skipping.`);
      continue;
    }
    ...
    try {
      ...
      console.warn(`Warning on slide ${slide.id}:`, e.message);
    }
  }

  console.log('✅ All 14 1080p slide frames verified!');
}
